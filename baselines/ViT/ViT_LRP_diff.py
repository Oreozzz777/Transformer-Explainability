...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False,attn_drop=0., proj_drop=0.):
        ...
        self.scale1 = self.head_dim ** -0.5
        self.scale2 = self.head_dim ** -0.5
    ...
    def relprop(self, cam, **kwargs):
        ...
        (cam_ldiff, cam_v) = self.matmul_a.relprop(cam, **kwargs)
        
        self.save_v_cam(cam_v)
        self.save_attn_cam(cam_ldiff)
        
        cam_l1 = cam_ldiff
        cam_l2_scaled = -cam_ldiff
        cam_l2 = cam_l2_scaled * self.lambda_param.data
        
        cam_l1 = self.attn_drop1.relprop(cam_l1, **kwargs)
        cam_l2 = self.attn_drop2.relprop(cam_l2, **kwargs)
        
        cam_s1 = self.softmax1.relprop(cam_l1, **kwargs)
        cam_s2 = self.softmax1.relprop(cam_l1, **kwargs)
        
        cam_s1 = cam_s1 * self.scale1
        cam_s2 = cam_s2 * self.scale2
        
        (cam_q1, cam_k1) = self.matmul_s1.relprop(cam_s1, **kwargs)
        (cam_q2, cam_k2) = self.matmul_s2.relprop(cam_s2, **kwargs)
        ...
        cam_x_k = self.k_proj.relprop(cam_k, **kwargs)
        cam_x_v = self.v_proj.relprop(cam_v, **kwargs)
        
        # Since q, k, v came from the same x, sum their relevances
        return cam_x_q + cam_x_k + cam_x_v
