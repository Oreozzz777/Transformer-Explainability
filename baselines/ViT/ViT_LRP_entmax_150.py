...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False,attn_drop=0., proj_drop=0., alpha=1.50):
        super().__init__()
        ...
        self.entmax = lambda x: entmax_bisect(x, alpha=self.alpha, dim=-1)
    ...
    def forward(self, x):
        ...
        attn = self.entmax(dots)
        attn = self.attn_drop(attn)

        # no_grad 모드에서도 동작하도록 보호
        self.save_attn(attn)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Attention blocks에도 enable_hooks 전달
